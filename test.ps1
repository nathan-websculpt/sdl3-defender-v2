...
$buildTarget = "SDL3Defender_tests"
Write-Host "Building target '$buildTarget' with preset: $buildPreset"
cmake --build --preset $buildPreset --target $buildTarget
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}

Write-Host "Running tests for config: $Config"
ctest --test-dir build-win-vcpkg -C $Config --output-on-failure
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}
